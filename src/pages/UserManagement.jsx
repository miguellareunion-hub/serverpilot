...
export default function UserManagement() {
  ...
  const { data: users = [], isLoading, refetch } = useQuery({ queryKey: ["users"], queryFn: () => base44.entities.User.list() });
  ...
  const handleInvite = async () => {
    ...
    setEmail(""); setRole("user"); setShowInvite(false); setInviting(false);
    refetch();
  };

  return (
    <div className="space-y-5">
      <PageHeader title="Utilisateurs" description="Gérez les accès à la plateforme" action="Inviter un utilisateur" icon={UserPlus} onAction={() => setShowInvite(true)} />
      ...
      {users.length === 0 && !isLoading ? (
        <EmptyState icon={Users} title="Aucun utilisateur" description="Invitez le premier utilisateur." action="Inviter" onAction={() => setShowInvite(true)} />
      ) : (
              ...
              <tbody>
                {users.map(user => (
                  <tr key={user.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    ...
                    <td className="p-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${user.role === "admin" ? "bg-indigo-500/15 text-indigo-400 border-indigo-500/30" : "bg-slate-500/15 text-slate-400 border-slate-500/30"}`}>
        ...
        <DialogContent className="bg-[#111827] border-white/10 text-white max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input value={email} onChange={e => setEmail(e.target.value)} type="email" className="bg-white/5 border-white/10 text-white" placeholder="user@example.com" />
            </div>
            <div>
