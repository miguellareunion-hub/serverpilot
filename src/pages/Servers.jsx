...
import { Server, Wifi, WifiOff, Pencil, Trash2, Plus, CheckCircle2, XCircle, Tag, Power } from "lucide-react";
...
export default function Servers() {
  ...
  const testConnection = async (server) => {
    ...
    await updateMutation.mutateAsync({
      id: server.id,
      data: { status: success ? "online" : "offline", last_checked: new Date().toISOString() }
    });
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {!server.enabled && (
                        <span className="text-[10px] bg-slate-700/40 text-slate-400 px-2 py-0.5 rounded-full border border-white/5">Désactivé</span>
                      )}
                    </div>
                    ...
                    {server.last_checked && (
                      <p className="text-[10px] text-slate-600 mt-0.5">
                        Vérifié : {new Date(server.last_checked).toLocaleString("fr-FR")}
                      </p>
                    )}
                  </div>
                </div>
        ...
        <AlertDialogContent className="bg-[#111827] border-white/10">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-400">
              Supprimer « {deleting?.name} » ? Tous les projets et données associés seront également affectés.
