...
export default function Backups() {
  ...
  const { data: backups = [], isLoading } = useQuery({ queryKey: ["backups"], queryFn: () => base44.entities.Backup.list("-created_date") });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["backups"] }); toast.success("Supprimée."); },
  });

  const handleCreate = async () => {
    const name = form.name || `Backup ${new Date().toLocaleDateString("fr-FR")}`;
    const backup = await createMutation.mutateAsync({ ...form, name, status: "running" });
    toast.success("Sauvegarde lancée !");
    setShowForm(false);
    setForm({ name: "", type: "project", server_id: "", project_id: "" });
    setTimeout(() => {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Sauvegardes" description="Gérez les sauvegardes de vos projets" action="Nouvelle sauvegarde" onAction={() => setShowForm(true)} />

      {backups.length === 0 && !isLoading ? (
        <EmptyState icon={Archive} title="Aucune sauvegarde" description="Créez votre première sauvegarde." action="Créer" onAction={() => setShowForm(true)} />
      ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {typeLabels[backup.type]}{backup.size && ` · ${backup.size}`}
                      </p>
                      <p className="text-[10px] text-slate-600 mt-0.5">{new Date(backup.created_date).toLocaleString("fr-FR")}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 flex-wrap">
                    {backup.status === "completed" && (
                      <Button size="sm" variant="ghost" onClick={() => toast.info("Restauration simulée")} className="text-indigo-400 hover:bg-indigo-500/10 gap-1 text-xs">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => setDeleting(backup)} className="text-rose-400 hover:bg-rose-500/10">
        ...
        <DialogContent className="bg-[#111827] border-white/10 text-white max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} className="bg-white/5 border-white/10 text-white" placeholder="Généré automatiquement" />
            </div>
            <div>
