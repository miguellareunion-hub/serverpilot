...
export default function Dependencies() {
  ...
  const handleAction = async (dep, action) => {
    ...
    if (action === "install" || action === "update") {
      setTimeout(async () => {
        await updateMutation.mutateAsync({ id: dep.id, data: { status: "installed", version: `${Math.floor(Math.random() * 20)}.${Math.floor(Math.random() * 9)}.0` } });
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {missingCount > 0 && (
            <Button size="sm" className="bg-amber-600 hover:bg-amber-500 text-white gap-2 text-xs">
      ...
      <div className="flex flex-wrap gap-3">
        <Select value={selectedServer} onValueChange={setSelectedServer}>
          <SelectTrigger className="w-44 bg-[#111827] border-white/10 text-white text-sm"><SelectValue placeholder="Tous les serveurs" /></SelectTrigger>
        ...
        <Input value={search} onChange={e => setSearch(e.target.value)} className="w-48 bg-[#111827] border-white/10 text-white text-sm" placeholder="Rechercher..." />
      </div>
      ...
      <div className="bg-[#111827] border border-white/5 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={4} className="p-8 text-center text-slate-500 text-sm">
                    ...
                    <td className="p-4">
                      <div className="flex items-center justify-end gap-1">
                        {dep.status === "not_installed" && (
                          <Button size="sm" variant="ghost" onClick={() => handleAction(dep, "install")} className="text-emerald-400 hover:bg-emerald-500/10 gap-1 text-xs">
                            <Download className="h-3.5 w-3.5" /> Installer
                          </Button>
                        )}
                        {dep.status === "installed" && (
                          <>
                            <Button size="sm" variant="ghost" onClick={() => handleAction(dep, "update")} className="text-indigo-400 hover:bg-indigo-500/10 gap-1 text-xs">
                              <RefreshCw className="h-3.5 w-3.5" /> M.à.j
                            </Button>
                            <Button size="sm" variant="ghost" onClick={() => handleAction(dep, "uninstall")} className="text-rose-400 hover:bg-rose-500/10 gap-1 text-xs">
                        ...
                        <Button size="sm" variant="ghost" onClick={() => deleteMutation.mutate(dep.id)} className="text-slate-600 hover:text-rose-400 hover:bg-rose-500/10">
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      </div>
                    </td>
        ...
        <DialogContent className="bg-[#111827] border-white/10 text-white max-w-sm">
          ...
          <div className="space-y-3 py-2">
            <div>
              ...
              <Input value={addForm.name} onChange={e => setAddForm(f => ({...f, name: e.target.value}))} className="bg-white/5 border-white/10 text-white" placeholder="nginx, redis..." />
            </div>
            <div>
              <Label className="text-slate-300 text-xs mb-1 block">Émoji</Label>
              <Input value={addForm.icon} onChange={e => setAddForm(f => ({...f, icon: e.target.value}))} className="bg-white/5 border-white/10 text-white" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowAdd(false)} className="text-slate-400">Annuler</Button>
            <Button onClick={async () => { await createMutation.mutateAsync({ ...addForm, status: "not_installed" }); toast.success("Ajouté !"); setShowAdd(false); setAddForm({ name: "", icon: "📦", server_id: "" }); }}
              disabled={!addForm.name} className="bg-indigo-600 hover:bg-indigo-500 text-white">Ajouter</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
