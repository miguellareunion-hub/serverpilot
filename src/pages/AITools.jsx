...
export default function AITools() {
  ...
  const { data: tools = [], isLoading } = useQuery({ queryKey: ["ai-tools"], queryFn: () => base44.entities.AITool.list() });
  ...
  const handleSave = async () => {
    ...
    if (editing) { await updateMutation.mutateAsync({ id: editing.id, data }); toast.success("Mis à jour !"); }
  ...
  const testConnection = async (tool) => {
    ...
    toast[tool.api_key || tool.endpoint ? "success" : "error"](tool.api_key || tool.endpoint ? `${tool.name} : connexion OK` : `${tool.name} : clé API manquante`);
    setTesting(null);
  };

  return (
    <div className="space-y-5">
      <PageHeader title="Outils IA" description="Configurez et gérez vos providers IA" action="Ajouter un provider" icon={Plus} onAction={openAdd} />

      {tools.length === 0 && !isLoading ? (
        <EmptyState icon={Brain} title="Aucun outil IA" description="Ajoutez un provider IA (OpenAI, Gemini, Ollama...)." action="Ajouter" onAction={openAdd} />
      ) : (
              ...
              <div key={tool.id} className={`bg-[#111827] border rounded-2xl p-4 transition-all ${tool.enabled ? 'border-white/5 hover:border-white/10' : 'border-white/[0.03] opacity-60'}`}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <div className="h-9 w-9 rounded-xl bg-violet-500/10 flex items-center justify-center text-lg">{prov.icon}</div>
                    <div>
                      <div className="flex items-center gap-1.5">
                        <h3 className="text-sm font-semibold text-white">{tool.name}</h3>
                        {tool.is_default && <span className="text-[9px] bg-indigo-500/20 text-indigo-400 px-1.5 py-0.5 rounded border border-indigo-500/30">Défaut</span>}
                      </div>
                      <span className={`text-[10px] ${tool.type === "local" ? "text-emerald-400" : "text-cyan-400"}`}>{tool.type === "local" ? "🏠 Local" : "☁️ Cloud"}</span>
                    </div>
                  </div>
                  <Switch checked={tool.enabled} onCheckedChange={v => updateMutation.mutate({ id: tool.id, data: { enabled: v } })} />
                </div>
                {tool.default_model && <p className="text-xs text-slate-500 mb-2">Modèle: <span className="text-slate-400 font-mono">{tool.default_model}</span></p>}
                <div className="flex gap-1 flex-wrap">
                  <Button size="sm" variant="ghost" onClick={() => testConnection(tool)} disabled={testing === tool.id} className="text-emerald-400 hover:bg-emerald-500/10 gap-1 text-xs">
                    <Wifi className="h-3.5 w-3.5" /> {testing === tool.id ? "Test..." : "Tester"}
                  </Button>
                  {!tool.is_default && (
                    <Button size="sm" variant="ghost" onClick={async () => {
                      for (const t of tools) { if (t.is_default && t.id !== tool.id) await updateMutation.mutateAsync({ id: t.id, data: { is_default: false } }); }
                      await updateMutation.mutateAsync({ id: tool.id, data: { is_default: true } }); toast.success("Provider par défaut défini");
                    }} className="text-indigo-400 hover:bg-indigo-500/10 gap-1 text-xs">
                      <Zap className="h-3.5 w-3.5" /> Défaut
                    </Button>
                  )}
                  <Button size="sm" variant="ghost" onClick={() => openEdit(tool)} className="text-slate-400 hover:bg-white/5"><Pencil className="h-3.5 w-3.5" /></Button>
                  <Button size="sm" variant="ghost" onClick={() => setDeleting(tool)} className="text-rose-400 hover:bg-rose-500/10"><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </div>
            );
          })}
        ...
        <DialogContent className="bg-[#111827] border-white/10 text-white max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Select value={form.provider} onValueChange={v => { const p = PROVIDERS.find(pr => pr.value === v); setForm(f => ({ ...f, provider: v, name: p?.label || v, type: p?.type || "cloud" })); }}>
            ...
            <div>
              ...
              <div className="relative">
                <Input type={showKey ? "text" : "password"} value={form.api_key || ""} onChange={e => setForm(f => ({...f, api_key: e.target.value}))} className="bg-white/5 border-white/10 text-white pr-9" placeholder="sk-..." />
                <button type="button" onClick={() => setShowKey(!showKey)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white">
            ...
            <div>
              ...
              <Input value={form.endpoint || ""} onChange={e => setForm(f => ({...f, endpoint: e.target.value}))} className="bg-white/5 border-white/10 text-white" placeholder="http://localhost:11434" />
            </div>
            <div>
              <Label className="text-slate-300 text-xs mb-1 block">Modèle par défaut</Label>
              <Input value={form.default_model || ""} onChange={e => setForm(f => ({...f, default_model: e.target.value}))} className="bg-white/5 border-white/10 text-white" placeholder="gpt-4o, gemini-pro, llama3..." />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowForm(false)} className="text-slate-400">Annuler</Button>
            <Button onClick={handleSave} className="bg-indigo-600 hover:bg-indigo-500 text-white">{editing ? "Mettre à jour" : "Ajouter"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
