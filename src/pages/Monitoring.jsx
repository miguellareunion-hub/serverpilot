...
export default function Monitoring() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select value={String(interval)} onValueChange={v => setInterval_(Number(v))}>
            <SelectTrigger className="w-32 bg-[#111827] border-white/10 text-white text-sm"><SelectValue /></SelectTrigger>
            <SelectContent className="bg-[#1e293b] border-white/10">
      ...
      {/* Main chart */}
      <div className="bg-[#111827] border border-white/5 rounded-2xl p-5">
        <h3 className="text-sm font-semibold text-white mb-4">Historique (48 points)</h3>
        <div className="h-56">
      ...
      {servers.length > 0 && (
        <div className="bg-[#111827] border border-white/5 rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-white/5">
            <h3 className="text-sm font-semibold text-white">Par serveur</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left p-3 text-xs text-slate-500 uppercase">Serveur</th>
                  <th className="text-left p-3 text-xs text-slate-500 uppercase">CPU</th>
                  <th className="text-left p-3 text-xs text-slate-500 uppercase">RAM</th>
                  <th className="text-left p-3 text-xs text-slate-500 uppercase">Disque</th>
                </tr>
              </thead>
              <tbody>
                {servers.map(s => (
                  <tr key={s.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    <td className="p-3 text-sm font-medium text-white">{s.name}</td>
                    {[s.cpu_usage, s.ram_usage, s.disk_usage].map((v, i) => (
                      <td key={i} className="p-3">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-white/5 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${i === 0 ? 'bg-indigo-500' : i === 1 ? 'bg-emerald-500' : 'bg-amber-500'}`}
                              style={{ width: `${v || 0}%` }} />
                          </div>
