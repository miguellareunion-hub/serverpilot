...
import { ScrollText, Trash2, Plus } from "lucide-react";
...
export default function Logs() {
  ...
  const { data: logs = [], isLoading } = useQuery({ queryKey: ["logs"], queryFn: () => base44.entities.LogEntry.list("-created_date", 200) });
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <Select value={categoryFilter} onValueChange={setCategoryFilter}>
          ...
          <SelectContent className="bg-[#1e293b] border-white/10">
            ...
            {["server","project","dependency","auth","backup","ai","system"].map(c => <SelectItem key={c} value={c} className="text-white">{c}</SelectItem>)}
          </SelectContent>
        </Select>
        <Input value={search} onChange={e => setSearch(e.target.value)} className="w-44 bg-[#111827] border-white/10 text-white text-sm" placeholder="Rechercher..." />
      </div>
              ...
              <tbody>
                {filtered.map(log => (
                  <tr key={log.id} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                    <td className="p-3 text-xs text-slate-500 whitespace-nowrap">{new Date(log.created_date).toLocaleString("fr-FR")}</td>
                    <td className="p-3"><StatusBadge status={log.level} /></td>
                    <td className="p-3 hidden sm:table-cell"><span className="text-xs text-slate-400 bg-white/5 px-2 py-0.5 rounded">{log.category}</span></td>
                    <td className="p-3">
        ...
        <DialogContent className="bg-[#111827] border-white/10 text-white max-w-md">
          ...
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={form.level} onValueChange={v => setForm(f => ({...f, level: v}))}>
                  ...
                  <SelectContent className="bg-[#1e293b] border-white/10">
                    {["info","warning","error","success"].map(l => <SelectItem key={l} value={l} className="text-white">{l}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div>
            ...
            <div>
              ...
              <Input value={form.message} onChange={e => setForm(f => ({...f, message: e.target.value}))} className="bg-white/5 border-white/10 text-white" />
            </div>
            <div>
              <Label className="text-slate-300 text-xs mb-1 block">Détails</Label>
              <Textarea value={form.details} onChange={e => setForm(f => ({...f, details: e.target.value}))} className="bg-white/5 border-white/10 text-white h-16" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setShowAdd(false)} className="text-slate-400">Annuler</Button>
            <Button onClick={async () => { await createMutation.mutateAsync(form); setShowAdd(false); }} disabled={!form.message} className="bg-indigo-600 hover:bg-indigo-500 text-white">Ajouter</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
