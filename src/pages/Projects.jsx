...
import { FolderGit2, Play, Square, RotateCcw, Trash2, Pencil, Download } from "lucide-react";
...
const typeColor = { nodejs: "emerald", python: "blue", docker: "cyan", static: "slate", other: "violet" };
        ...
        <div className="grid gap-3">
          {projects.map(project => (
            <div key={project.id} className="bg-[#111827] border border-white/5 rounded-2xl p-4 hover:border-white/10 transition-all">
                  ...
                  <div>
                    ...
                    {project.last_deploy_at && (
                      <p className="text-[10px] text-slate-600 mt-0.5">
                        Déployé : {new Date(project.last_deploy_at).toLocaleString("fr-FR")}
                      </p>
                    )}
                  </div>
                </div>
              ...
              {project.description && (
                <p className="text-xs text-slate-500 mt-2 ml-[52px] line-clamp-1">{project.description}</p>
              )}
            </div>
          ))}
        </div>
        ...
        <AlertDialogContent className="bg-[#111827] border-white/10">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-400">
              Supprimer « {deleting?.name} » (port {deleting?.port}) ? Cette action est irréversible.
